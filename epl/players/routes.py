...
def new_player():
    ...
    if request.method == 'POST':
        ...
        if position == 'Goalkeeper':
            ...
            val = request.form.get('clean_sheets')
            clean_sheets = int(val) if val else 0
...
def search_player():
    ...
    return render_template('players/search_player.html', title='Search Player Page', players=[])
...
def update_player(id):
    ...
    if request.method == 'POST':
        ...
        if position == 'Goalkeeper':
            val = request.form.get('clean_sheets')
            player.clean_sheets = int(val) if val else 0
        else:
